...
function WeavePlayerValue:getNow()
...
function WeavePlayerValue:getNowFor(player: Player)
	if RunService:IsClient() then
...
function WeavePlayerValue:Fusion()
...
function WeavePlayerValue:updateFor(player: Player, func: (t: any) -> nil, force: boolean)
	if RunService:IsClient() then
		error("Cannot set WeavePlayerValue value on the client. Only the Server")
		return
	end
...
function WeavePlayerValue<T>(valueName: string, initialValue: T?)
