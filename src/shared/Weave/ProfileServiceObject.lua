...
local WeaveObject = require(script.Parent.WeaveObject)
...
local function ProfileServiceObject<T>(valueName: string, profileServiceKey: string?)
	
	if RunService:IsClient() then
		return WeaveObject(`profileServiceObject_{valueName}`)
	end
	...
	local self = setmetatable({
		...
		_name = valueName,
		_playerObject = WeaveObject(`profileServiceObject_{valueName}`, {}),
		_troves = {}
	}, CLASS_METATABLE)
